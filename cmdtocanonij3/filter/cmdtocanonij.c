...
#define CMD_AUTO_ALIGNMENT	(2)

#define UTIL_CMD_TEST_PRINT     ("PrintSelfTestPage")
#define UTIL_CMD_CLEANING       ("Clean all")
#define UTIL_CMD_AUTO_ALIGNMENT ("com.canon.autoalignment")
...
static  int		(*MakeCommand_mnt_AutoAlignment)(char[], void *, int, long *);
...
int MakeCommand_AutoAlignment( uint8_t *cmdBuf, long cmdSize, long *writtenSize ){
	int rtn = PROC_FAILED;
	// long writtenSize = 0;


	MakeCommand_mnt_AutoAlignment = dlsym( _libclss, "CNCL_MakeCommand_AutoAlignment" );

	if ( dlerror() != NULL ) {
		fprintf( stderr, "[cmdtocanonij3] Load Error in CNCL_MakeCommand_AutoAlignment\n" );
		rtn = ERR_MAKE_COMMAND;
		goto error_exit;
	}

	rtn = MakeCommand_mnt_AutoAlignment( CN_START_JOBID2, cmdBuf, cmdSize, writtenSize );

	if ( rtn != PROC_SUCCEEDED ) {
		fprintf( stderr, "[cmdtocanonij3] Error in CNCL_MakeCommand_AutoAlignment\n" );
		rtn = ERR_MAKE_COMMAND;
		goto error_exit;
	}
...
int getMaintenanceType( int ifd, int *type ){
	...
	for( lines = 0; ( read_bytes = read_line( ifd, read_buf, DATA_BUF_SIZE - 1 ) ) > 0; lines++ ){
		...
		else if( strncmp( read_buf, UTIL_CMD_AUTO_ALIGNMENT, strlen( read_buf ) - 1 ) == 0 ){
			*type = CMD_AUTO_ALIGNMENT;
			rtn = PROC_SUCCEEDED;
			break;
		}
		else{
...
int main( int argc, char *argv[] ){
	...
	MakeCommand_mnt_AutoAlignment = NULL;
...
// >>>>>>> parent of 1f4dac4... Test : cmdtocanonij3
	...
	if( rtn == PROC_SUCCEEDED ){
		...
		else if( dType == CMD_AUTO_ALIGNMENT ){
			// make command (AutoAlignment)
			rtn = MakeCommand_AutoAlignment( cmdBuf, CN_BUFSIZE, &writtenSize );
